...
# 🔮 НАДЁЖНЫЙ ГЕНЕРАТОР ГОРОСКОПОВ (замена внешнему API)
def get_horoscope_reliable(sign):
    """Генерирует уникальный гороскоп на день для каждого знака без внешних запросов"""
    today = datetime.now().strftime("%Y-%m-%d")
    seed_str = f"{today}_{sign}"
    seed = int(hashlib.md5(seed_str.encode()).hexdigest(), 16)

    intros = [
        "Звёзды сегодня благоволят твоим начинаниям.",
        "Энергия дня направлена на внутренние перемены.",
        "Космические вибрации усиливают интуицию.",
        "Сегодня день для стратегических решений.",
        "Вселенная подсказывает: не торопи события.",
        "Планетарный аспект открывает новые возможности."
    ]
    activities = [
        "завершения старых дел", "новых знакомств", "творческого самовыражения",
        "финансового планирования", "отдыха и восстановления", "обучения"
    ]
    warnings = [
        "Избегай импульсивных трат.", "Не распыляйся на мелочи.",
        "Дай близким больше внимания.", "Проверяй документы дважды.",
        "Следи за режимом сна.", "Не вовлекайся в чужие конфликты."
    ]
    colors = ["🟢 Зелёный", "🔵 Синий", "🟠 Оранжевый", "🟣 Фиолетовый", "⚪ Белый", "🔴 Красный"]
    times = ["10:00-12:00", "14:00-16:00", "18:00-20:00", "09:00-11:00", "15:00-17:00"]

    intro = intros[seed % len(intros)]
    activity = activities[(seed >> 8) % len(activities)]
    warning = warnings[(seed >> 16) % len(warnings)]
    color = colors[(seed >> 24) % len(colors)]
    time = times[(seed >> 12) % len(times)]

    return f"🔮 {intro}\n\n✨ Благоприятно для: {activity}.\n⚠️ Совет: {warning}\n🍀 Цвет: {color} | ⏰ Время: {time}"
...
def get_astro_tip():
    return random.choice(["🌿 Луна в знаке Земли: время для планирования бюджета.", "⚡ Меркурий активен: не отправляй важные сообщения после 22:00.", "🌊 Ретроградный период: перепроверяй билеты.", "☀️ Солнечный аспект: время для новых проектов.", "🌙 Лунный день: очищение пространства и мыслей.", "🔮 Венера в гармонии: день для творчества."])
...
async def show_horoscope(cb: types.CallbackQuery):
    ...
    text = get_horoscope_reliable(sign)  # ← ИСПОЛЬЗУЕМ НАДЁЖНЫЙ ГЕНЕРАТОР
